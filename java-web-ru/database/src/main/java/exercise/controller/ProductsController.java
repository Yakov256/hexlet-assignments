...
public class ProductsController {
    ...
    public static void create(Context ctx) throws SQLException {
        try {
            var title = ctx.formParamAsClass("title", String.class)
                .check(value -> !value.isEmpty(), "Название не должно быть пустым").get();
            var price = ctx.formParamAsClass("price", Integer.class)
                .check(value -> value >= 0, "Цена не должна быть отрицательной").get();
    ...
    public static void show(Context ctx) throws SQLException {
        ...
        var product = ProductsRepository.find(id).orElseThrow(() -> new NotFoundResponse("Product not found"));
